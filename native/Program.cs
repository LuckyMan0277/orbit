...
namespace Orbit {
    ...
    internal sealed partial class MainWindow : Form, IRemoteTerminals, IRemoteSecrets, IRemoteUploads, IDeskBridge {
        ...
        private readonly ConcurrentDictionary<string,string> sessionProjects=new ConcurrentDictionary<string,string>();
        ...
        // project / seq / lastOutputMs / asking let a phone show each session's state (working, new output, waiting for a key) without watching every one.
        object IRemoteTerminals.Sessions() { return new {sessions=sessions.Values.Select(x=> { string profile,name,project; if(!sessionProfiles.TryGetValue(x.Id,out profile))profile="terminal"; if(!sessionNames.TryGetValue(x.Id,out name))name=profile; if(!sessionProjects.TryGetValue(x.Id,out project))project="";OutputRing ring;long seq=0,ticks=0;if(outputRings.TryGetValue(x.Id,out ring))ring.Stats(out seq,out ticks);long ago=ticks==0?-1:(long)Math.Max(0,(DateTime.UtcNow-new DateTime(ticks,DateTimeKind.Utc)).TotalMilliseconds);return new {id=x.Id,name=name,pid=x.ProcessId,profile=profile,project=project,seq=seq,lastOutputMs=ago,asking=broker.PendingList(x.Id).Length>0};}).ToArray()}; }
