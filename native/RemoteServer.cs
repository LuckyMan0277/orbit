...
namespace Orbit {
 ...
 internal sealed class RemoteServer:IDisposable {
  ...
  void Handle(HttpListenerContext c) {
   ...
   if(op=="terminals/create"){string profile=S(a,"profile"),resumeId=S(a,"resumeId"),project=S(a,"project");if(profile!="codex"&&profile!="claude"&&profile!="powershell"&&profile!="cmd"){Reply(c,400,new {error="invalid profile"});return;}if(!String.IsNullOrEmpty(resumeId)&&((profile!="codex"&&profile!="claude")||!SessionHistory.IsId(resumeId))){Reply(c,400,new {error="invalid saved session"});return;}if(project.Length>1024){Reply(c,400,new {error="invalid project"});return;}string[] secrets=a.ContainsKey("secrets")?Names(a,"secrets"):null;var vault=terminals as IRemoteSecrets;if(vault!=null){Reply(c,200,vault.Create(profile,resumeId,project,secrets));return;}if(secrets!=null&&secrets.Length>0){Reply(c,400,new {error="secrets unavailable"});return;}Reply(c,200,terminals.Create(profile,resumeId,project));return;}
   if(op=="secrets"||op=="secrets/set"||op=="secrets/delete"||op=="secrets/answer"){SecretsRoute(c,a,op);return;}
   if(op=="snapshot"){object value=terminals.Snapshot(S(a,"session"));if(Auth(c)==null){Reply(c,401,new {error="revoked"});return;}Reply(c,200,WithEpoch(value,S(a,"session")));return;}
   if(op=="output"){object value=terminals.Output(S(a,"session"),L(a,"seq"),25000);if(Auth(c)==null){Reply(c,401,new {error="revoked"});return;}Reply(c,200,WithEpoch(value,S(a,"session")));return;}
   if(op=="input/status"){InputStatus(c,a,device);return;}
   if(op=="input"){Input(c,a,device);return;}
   Reply(c,404,new {error="unknown api"});
  }
  // Values are write-only: a request may store or delete one, but no response ever carries a value back.
  void SecretsRoute(HttpListenerContext c,Dictionary<string,object> a,string op){
   var vault=terminals as IRemoteSecrets;if(vault==null){Reply(c,404,new {error="unknown api"});return;}
   try{
    string project=S(a,"project");if(project.Length>1024){Reply(c,400,new {error="invalid project"});return;}
    if(op=="secrets"){Reply(c,200,vault.SecretNames(project));return;}
    if(op=="secrets/answer"){Reply(c,200,new {ok=vault.AnswerSecretRequest(S(a,"request"),S(a,"status"))});return;}
  ...
  object WithEpoch(object value,string session=null){var map=json.Deserialize<Dictionary<string,object>>(json.Serialize(value))??new Dictionary<string,object>();map["serverEpoch"]=serverEpoch;var secrets=session==null?null:terminals as IRemoteSecrets;if(secrets!=null)map["secretRequests"]=secrets.PendingSecretRequests(session);return map;}
