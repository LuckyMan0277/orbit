...
namespace Orbit {
 internal sealed class RemoteTunnel:IDisposable {
  readonly object gate=new object();Process process;Timer timeout;string url,error;bool starting;int generation;IntPtr job;
  ...
  void StopInternal(Process expected,int g,string why){Process p;IntPtr j;lock(gate){if(g!=generation||process!=expected)return;p=process;process=null;j=job;job=IntPtr.Zero;starting=false;url=null;error=why;if(timeout!=null){timeout.Dispose();timeout=null;}}try{if(p!=null&&!p.HasExited)p.Kill();}catch{}try{if(p!=null)p.Dispose();}catch{}if(j!=IntPtr.Zero)Win32.CloseHandle(j);Raise();}
