...
1. 왼쪽 위에서 작업 폴더를 선택합니다.
2. **저장된 대화**에서 이전 Codex·Claude 대화를 선택하거나, **새 Codex·새 Claude** 버튼으로 새 대화를 시작합니다. `Ctrl+Shift+T`는 기본 셸을 엽니다.
3. 탭을 끌어 순서를 바꾸거나 패널 가장자리에 놓아 분할합니다. 탭을 더블클릭하면 이름을 바꿀 수 있습니다.
4. **파일 탐색기** 탭에서 파일을 클릭하면 오른쪽 편집기에 열립니다. `Ctrl+S`로 저장합니다.
