...
async function newTerminal(profile = state.settings.defaultShell, resume = null) {
  ...
  const title = resume?.title?.trim().slice(0, 40) || null;
  const name = title || sessionName(projectName(cwd), p.name, sameProject), label = title || sessionLabel(p.name, sameProject);
