...
async function initialize() {
  ...
  if (isDesktop) { const updateRemoteBadge=s=>{const pending=s.pending?.length||0;$('#external-state').textContent=pending?'승인 '+pending:!s.enabled?'꺼짐':s.url?.startsWith('https:')?'외부 준비':'이 PC';};call('remoteStatus').then(updateRemoteBadge).catch(()=>{});on('remoteStatus',data=>updateRemoteBadge(data.status)); } else { $('#status-text').textContent = '터미널 실행은 데스크톱 앱에서'; }
  if (isDesktop && state.settings.remoteAutoConnect && !data.testMode) call('remoteConnectStart',{port:49821}).then(result=>{const s=result?.status;if(s){const badge=$('#external-state');badge.textContent=s.pending?.length?`승인 ${s.pending.length}`:!s.enabled?'꺼짐':s.url?.startsWith('https:')?'외부 준비':'이 PC';}}).catch(error=>{const badge=$('#external-state');badge.textContent='연결 확인';badge.title=`자동 연결 실패: ${error.message}. 클릭하여 다시 연결하세요.`;$('#external-connection').title=badge.title;});
...
async function remoteDevices(){
  let status=await call('remoteStatus'),tunnel=await call('remoteTunnelStatus').catch(()=>({})),tailscale=await call('remoteTailscaleStatus').catch(()=>({})),qrMaking=false,connecting=false,closed=false,epoch=0,expiryTimer,inviteId='',connectionError=tailscale.error||tunnel.error||'';
  let render=()=>{},clearInvite=()=>{},makeInvite=async()=>{},autoInvite=!!(tunnel.starting||tailscale.starting);const merge=result=>{if(result?.status)status=result.status;if(result?.tunnel)tunnel=result.tunnel;if(result?.tailscale)tailscale=result.tailscale;};const unsub=on('remoteStatus',data=>{if(!closed){if(status.url!==data.status.url){epoch++;clearInvite();}status=data.status;render();}});const tunnelUnsub=on('remoteTunnel',data=>{if(closed)return;tunnel=data.tunnel||{url:data.url,error:data.error,starting:false};connecting=!!tunnel.starting;if(data.error)connectionError=data.error;if(data.url){const url=data.url.replace(/\/$/,'')+'/mobile.html';if(status.url!==url){epoch++;clearInvite();status={...status,url};}}render();if(data.url&&autoInvite){autoInvite=false;makeInvite().catch(e=>{connectionError=e.message;render();});}});const tailscaleUnsub=on('remoteTailscale',data=>{if(closed)return;tailscale=data.tailscale||{};connecting=!!tailscale.starting;if(tailscale.error)connectionError=tailscale.error;render();if(tailscale.url&&autoInvite){autoInvite=false;makeInvite().catch(e=>{connectionError=e.message;render();});}});
  try { await dialog('외부 연결',node=>{
    const intro=el('p','remote-intro'),summary=el('p','muted'),error=el('p','dialog-note'),setupNote=el('p','dialog-note'),setupActions=el('div','remote-actions remote-setup-actions'),layout=el('div','remote-dialog-layout'),qrSection=el('section','remote-qr-section'),qrTitle=el('h3','', '새 기기 등록'),url=el('input','dialog-input'),qr=el('div','remote-qr'),qrActions=el('div','remote-actions'),validity=el('p','dialog-note'),devicesSection=el('section','remote-devices-section'),devicesTitle=el('h3','', '등록된 기기'),list=el('div','remote-device-list'),rightColumn=el('div','remote-dialog-side'),advanced=document.createElement('details');
    makeInvite=async()=>{if(qrMaking||!status.url?.startsWith('https:'))return;const mine=++epoch;clearInvite();qrMaking=true;render();try{const r=await call('remoteInvite',{name:'mobile'});const link=r.url+'#pair='+encodeURIComponent(r.invite);if(closed||mine!==epoch){call('remoteInviteCancel',{id:r.id}).catch(()=>{});return;}inviteId=r.id;url.value=link;armExpiry();await drawQr(link,mine);}finally{qrMaking=false;if(!closed)render();}};
    const start=button('외부 주소 준비','primary',guard(async()=>{if(connecting||qrMaking)return;connectionError='';connecting=true;autoInvite=true;epoch++;clearInvite();render();try{merge(await call('remoteConnectStart',{port:49821}));state.settings.remoteAutoConnect=true;persist();if(status.url?.startsWith('https:')){autoInvite=false;await makeInvite();}}catch(e){autoInvite=false;connectionError=e.message;throw e;}finally{connecting=!!(tunnel.starting||tailscale.starting);if(!closed)render();}})),copy=button('링크 복사','secondary',guard(async()=>{if(url.value){await call('clipboard',{text:url.value});toast('초대 링크를 복사했습니다. 5분 안에 등록하세요.');}})),stop=button('외부 접속 끄기','secondary',guard(async()=>{connectionError='';autoInvite=false;state.settings.remoteAutoConnect=false;await persist();await call('remoteStop',{});status=await call('remoteStatus');tunnel=await call('remoteTunnelStatus').catch(()=>({}));tailscale=await call('remoteTailscaleStatus').catch(()=>({}));clearInvite();render();}));start.id='remote-connect-start';
    url.readOnly=true;url.placeholder='QR을 만들면 등록 링크가 표시됩니다.';qr.hidden=true;validity.textContent='QR은 새 기기 하나를 등록하는 데만 쓰이며 5분 후 만료됩니다. 새 QR을 만들거나 주소가 바뀌면 이전 QR은 취소됩니다.';qrActions.append(copy);clearInvite=(cancel=true)=>{clearTimeout(expiryTimer);if(cancel&&inviteId){const id=inviteId;call('remoteInviteCancel',{id}).catch(()=>{});}inviteId='';url.value='';delete qr.dataset.svg;qr.replaceChildren();qr.hidden=true;};const armExpiry=()=>{clearTimeout(expiryTimer);expiryTimer=setTimeout(()=>{if(!closed){++epoch;clearInvite();toast('등록 QR이 만료되었습니다. 새로 만드세요.');render();}},300000);};async function drawQr(link,mine){const {default:qrcode}=await import('qrcode-generator');if(closed||mine!==epoch)return;const code=qrcode(0,'M');code.addData(link);code.make();if(closed||mine!==epoch)return;const svg=code.createSvgTag({cellSize:4,margin:16,scalable:true}).replace('<svg ','<svg style="background:white" ').replace('stroke="transparent" fill="black"','stroke="none" fill="black"');const source=new Image();source.src='data:image/svg+xml;charset=utf-8,'+encodeURIComponent(svg);await source.decode();const canvas=document.createElement('canvas');canvas.width=source.naturalWidth;canvas.height=source.naturalHeight;const context=canvas.getContext('2d');context.fillStyle='#fff';context.fillRect(0,0,canvas.width,canvas.height);context.drawImage(source,0,0);const image=new Image();image.alt='새 기기 등록 QR';image.src=canvas.toDataURL('image/png');await image.decode();if(closed||mine!==epoch)return;qr.dataset.svg=svg;qr.replaceChildren(image);qr.hidden=false;}
    const advancedSummary=el('summary','', '고급');const publicUrl=el('input','dialog-input');publicUrl.value=state.settings.remoteOrigin||'';publicUrl.placeholder='고정 공개 HTTPS 주소 (선택)';publicUrl.onchange=guard(async()=>{epoch++;clearInvite();connectionError='';state.settings.remoteOrigin=publicUrl.value.trim();await call('settings',state.settings);await call('remotePublicOrigin',{url:state.settings.remoteOrigin});status=await call('remoteStatus');render();});const fallback=button('임시 Cloudflare 연결 사용','secondary',guard(async()=>{if(connecting)return;connectionError='';connecting=true;autoInvite=true;epoch++;clearInvite();render();try{merge(await call('remoteTunnelStart',{port:49821}));if(status.url?.startsWith('https:')){autoInvite=false;await makeInvite();}}finally{connecting=!!(tunnel.starting||tailscale.starting);render();}}));const diagnosis=button('연결 상태 확인','secondary',guard(async()=>{const result=await call('remoteDiagnostic');connectionError=result.checkedUrl?(result.reachable?'공개 모바일 주소를 확인했습니다.':('공개 모바일 주소에 연결하지 못했습니다. '+(result.error||''))):'현재 공개 HTTPS 주소가 없습니다.';render();}));const setup=button('Tailscale 로그인 계속','primary',guard(()=>call('remoteTailscaleSetup',{url:tailscale.setupUrl})));const install=button('Tailscale 설치 열기','primary',guard(()=>call('remoteTailscaleSetup',{url:'https://tailscale.com/download/windows'})));const advancedActions=el('div','remote-actions');advancedActions.append(fallback,diagnosis);const stopNote=el('p','dialog-note remote-stop-note','외부 접속을 끄면 현재 원격 연결은 종료됩니다. 이미 승인한 기기와 이 PC의 터미널은 그대로 남습니다.');advanced.append(advancedSummary,el('p','dialog-note','고정 공개 HTTPS 주소를 직접 넣거나 임시 Cloudflare 주소로 바꿀 수 있습니다. 주소가 준비되지 않거나 접속이 안 될 때만 사용하세요.'),publicUrl,advancedActions,stopNote,stop);setupActions.append(setup,install);
    render=()=>{const https=status.url?.startsWith('https:');const completed=(status.completed||[]).find(p=>p.id===inviteId);if(completed&&inviteId){clearInvite(false);connectionError='등록 완료 · '+completed.name;}if(!https&&inviteId)clearInvite();const waiting=connecting||tunnel.starting||tailscale.starting;const needsLogin=!https&&!!tailscale.setupUrl&&!waiting;const needsInstall=!https&&!tailscale.setupUrl&&/설치되어 있지 않습니다/.test(tailscale.error||'')&&!waiting;intro.textContent=https?'이 PC는 외부 접속을 받을 준비가 되었습니다.':'휴대폰이나 태블릿에서 이 PC의 작업 공간을 열 수 있습니다.';summary.textContent=waiting?(tunnel.starting?'임시 Cloudflare 주소를 준비하고 있습니다.':'외부 주소를 준비하고 있습니다.'):https?(inviteId?'휴대폰으로 QR을 스캔한 뒤, 이 PC에서 등록 요청을 승인하세요.':'새 기기를 등록하려면 QR을 만드세요.'):'외부 주소를 준비하면 도메인 없이 HTTPS 주소가 만들어집니다. Tailscale 승인이 필요하면 브라우저에서 마친 뒤 Orbit으로 돌아오세요.';setupNote.hidden=!(needsLogin||needsInstall);setupNote.textContent=needsLogin?'Tailscale 로그인이 필요합니다. 브라우저에서 승인한 뒤 Orbit으로 돌아오세요.':needsInstall?'Tailscale 설치가 필요합니다. 설치한 뒤 다시 외부 주소를 준비하세요.':'';error.hidden=!connectionError;error.textContent=connectionError?'연결 상태: '+connectionError:'';setup.hidden=!needsLogin;install.hidden=!needsInstall;setupActions.hidden=!(needsLogin||needsInstall);start.disabled=waiting||qrMaking;start.textContent=waiting?'외부 주소 준비 중…':https?'새 기기 QR 만들기':'외부 주소 준비';copy.disabled=!url.value;stop.disabled=!status.enabled;qrSection.hidden=!https;devicesSection.hidden=!https&&!(status.pending?.length||status.devices?.length);list.replaceChildren();for(const p of status.pending||[]){const row=el('div','remote-device-row'),details=el('div',''),actions=el('div','remote-actions');details.append(el('strong','',p.name),el('small','',p.approved?'휴대폰 연결 확인 중':'이 PC의 승인 대기'));const approve=button('승인','secondary',guard(async()=>{await call('remoteApprove',{id:p.id});status=await call('remoteStatus');render();}));approve.disabled=!!p.approved;actions.append(approve,button('거부','secondary',guard(async()=>{epoch++;clearInvite();await call('remoteReject',{id:p.id});status=await call('remoteStatus');render();})));row.append(details,actions);list.append(row);}for(const d of status.devices||[]){const row=el('div','remote-device-row'),details=el('div','');details.append(el('strong','',d.name),el('small','','등록됨'));row.append(details,button('등록 해제','secondary',guard(async()=>{await call('remoteRevoke',{id:d.id});status=await call('remoteStatus');render();})));list.append(row);}if(!list.childElementCount&&https)list.append(el('p','dialog-note','등록된 기기가 없습니다. QR로 첫 기기를 등록하세요.'));};
    stop.classList.add('remote-stop');qrSection.append(qrTitle,qr,url,qrActions,validity);devicesSection.append(devicesTitle,list);rightColumn.append(devicesSection,advanced);layout.append(qrSection,rightColumn);node.append(intro,summary,error,setupNote,start,setupActions,layout);render();
  });
  }finally{closed=true;++epoch;clearTimeout(expiryTimer);unsub();tunnelUnsub();tailscaleUnsub();}
}
