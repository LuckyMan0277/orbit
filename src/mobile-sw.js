...
self.addEventListener('fetch',event=>{const url=new URL(event.request.url);if(url.origin!==location.origin||url.pathname.startsWith('/api/')||event.request.method!=='GET')return;if(!allowed(url.pathname)){if(event.request.mode==='navigate')event.respondWith(caches.match('/mobile.html'));return;}event.respondWith(fetchWithTimeout(event.request,8000).then(response=>{if(response.ok)caches.open(CACHE).then(cache=>cache.put(event.request,response.clone()));return response;}).catch(()=>caches.match(event.request).then(hit=>{if(hit)return hit;throw new Error('offline asset unavailable');})));});
