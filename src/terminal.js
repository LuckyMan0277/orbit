...
export class TerminalPane {
  constructor({ id, profile, cwd, resumeId, name, settings, openLink, onExit, onFocus }) {
    ...
    this.element = el('div', 'terminal-pane');
    ...
    this.term.onResize(({ cols, rows }) => { if (this.started && !this.exited) notify('resize', { session: this.id, cols, rows }); });
    ...
    this.observer = new ResizeObserver(() => { if (this.element.clientWidth && this.element.clientHeight) this.fit.fit(); });
  ...
  focus() { requestAnimationFrame(() => { if (!this.closed && this.element.clientWidth) { this.fit.fit(); this.term.focus(); } }); }
  configure(settings) { this.term.options.fontSize = settings.fontSize; this.term.options.scrollback = settings.lowPower ? 500 : 2000; this.term.options.minimumContrastRatio = settings.theme === 'light' ? 4.5 : 1; this.term.options.theme = terminalTheme(settings.theme); this.fit.fit(); }
